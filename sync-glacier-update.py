...
if not len(dirs):
    print("You need to give the full path to a folder to sync in the second line of the config file, "
          "e.g. `C:\backups`. You can list multiple folders, e.g. `C:\backups|D:\backups`")
    terminate(1)
...
try:
    ...
    print("Transferred " + format_bytes(transferred) + " in " + format_time(elapsed) + " at rate of " + format_bytes(
        transferred / elapsed) + "/s.")
    terminate(0)
